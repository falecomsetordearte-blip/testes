// /impressao/painel-script.js - VERSÃO FINAL COM LÓGICA DE LIMPEZA DE MENSAGEM REPLICADA E CORRIGIDA
...
(function() {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function limparTextoMensagem(texto) {
            ...
            // 1. Remove cabeçalhos genéricos como [Autor]\n---\n
            let textoLimpo = texto.replace(/^\[.+?\]\n-+\n/, '');

            // 2. Remove todos os padrões de tags no início, incluindo números e [Message]
            // Ex: "55 [Message] Oi" -> "Oi"
            // Ex: "[ChatApp robot][WhatsApp] Oi" -> "Oi"
            textoLimpo = textoLimpo.replace(/^(?:\[.*?\]|\d+\s*)+(?:\[Message\]\s*)?/, '');
            
            // 3. Converte links de imagem BBCode em texto simples (apenas a URL)
            // Isso evita a exibição das tags [img]
            ...
            try {
                ...
                if (messages.length > 0) {
                    chatContainer.innerHTML = messages.map(msg => {
                        ...
                        if (!textoLimpo) return ''; // Ignora mensagens que ficaram vazias
                        return `<div class="mensagem ${classe}">${textoLimpo}</div>`;
                    }).join('');
                    chatContainer.scrollTop = chatContainer.scrollHeight;
                } else {
                    chatContainer.innerHTML = '<p class="info-text">Nenhuma mensagem ainda.</p>';
                }
            } catch (error) {
        ...
        function openDetailsModal(dealId) {
            ...
            modalBody.innerHTML = `<div class="steps-container">${stepsHtml}</div><div class="detalhe-layout"><div class="detalhe-col-principal">${mainColumnHtml}</div><div class="detalhe-col-lateral"><div class="card-detalhe">${actionsHtml}</div><div class="card-detalhe"><h3>Informações do Cliente</h3><div class="info-item"><span class="info-item-label">Nome:</span><span>${nomeCliente}</span></div><div class="info-item"><span class="info-item-label">Contato:</span><span>${contatoCliente}</span></div><div class="info-item"><span class="info-item-label">Medidas:</span>${medidasHtml}</div></div></div></div>`;
            ...
            if (approveBtn) {
                approveBtn.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        approveBtn.innerHTML = '<i class="fas fa-check"></i> Aprovar Arquivo';
                    }
                });
            }
        }
