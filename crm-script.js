// crm-script.js - VERSÃO FINAL INTEGRADA COM PEDIDOS REAIS E DIAS ÚTEIS
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    configurarMascaras();
    carregarKanban();
    carregarMetasCRM(); // Inicializa o sistema de metas
...
async function carregarMetasCRM() {
    ...
    try {
        ...
        if(globalMetasData && globalMetasData.metas) {
            atualizarDropdownMetas(globalMetasData.metas);
            renderizarVisualizacaoMeta();
        } else {
            document.getElementById('meta-text-left').innerHTML = `<span style="color:#e74c3c;">Metas não configuradas.</span>`;
            document.getElementById('meta-text-right').innerHTML = `<a href="/admin-metas.html" style="color:#3498db; font-weight:bold;">⚙️ Configurar</a>`;
        }
    } catch(e) {
...
function atualizarDropdownMetas(m) {
    const format = (dStr) => { 
        if(!dStr) return ''; 
...
// Calcula dias úteis (Segunda a Sexta) restantes do dia atual até o fim do mês
function calcularDiasUteisRestantes() {
    let count = 0;
    let hoje = new Date();
    let fimMes = new Date(hoje.getFullYear(), hoje.getMonth() + 1, 0);
    ...
    while(tempDate <= fimMes) {
        ...
        if(diaSemana !== 0 && diaSemana !== 6) count++; // Pula Sáb e Dom
...
window.renderizarVisualizacaoMeta = function() {
    ...
    if (filtro === 'diaria') {
        ...
        metaAlvo = diasRestantes > 0 ? (faltaParaOMes / diasRestantes) : 0;
        atual = total_hoje;
        textoEsq = `Hoje: ${fmt(atual)}`;

    } else if (filtro === 'mensal') {
        metaAlvo = Number(metas.meta_mensal);
        atual = total_mes;
        textoEsq = `Mês: ${fmt(atual)}`;
        premio = metas.premio_mensal;

    } else {
        // Semanas (Usa o valor isolado calculado pela API)
        ...
        textoEsq = `Vendido na Semana: ${fmt(atual)}`;
    }

    let porcentagem = metaAlvo > 0 ? (atual / metaAlvo) * 100 : 0;
    if(porcentagem > 100) porcentagem = 100;

    document.getElementById('meta-text-left').innerText = textoEsq;
    document.getElementById('meta-text-right').innerText = `Meta: ${fmt(metaAlvo)} (${porcentagem.toFixed(1)}%)`;
...
// --- RESTANTE DO SCRIPT CRM (KANBAN, BUSCA, ETC) ---
...
function configurarBuscaCliente() {
    ...
    const performSearch = async (e) => {
        ...
        if(term.length < 2) return showToast("Digite pelo menos 2 caracteres.", "error");
...
window.produzirCardDireto = function(cardId, btnElement) {
    ...
    if (btnElement.dataset.confirming === "true") {
        ...
        let extras = {}; try { if(card.briefing_json) extras = (typeof card.briefing_json === 'string') ? JSON.parse(card.briefing_json) : card.briefing_json; } catch(e){}
        let txt = ""; if(extras.materiais) extras.materiais.forEach((d, i) => { txt += `--- Item ${i+1} ---\nMaterial: ${d.descricao}\nDetalhes: ${d.detalhes}\n\n`; });
        const payload = { sessionToken: localStorage.getItem('sessionToken'), titulo: card.titulo_automatico, servico: card.servico_tipo, arte: card.arte_origem, nomeCliente: card.nome_cliente, wppCliente: card.wpp_cliente, tipoEntrega: extras.tipo_entrega, briefingFormatado: txt, linkArquivo: extras.link_arquivo, supervisaoWpp: extras.supervisao_wpp, valorDesigner: extras.valor_designer, formato: extras.formato, cdrVersao: extras.cdr_versao };
        enviarProducaoAPI(payload, cardId, btnElement);
    } else {
        btnElement.dataset.confirming = "true"; btnElement.innerHTML = '<i class="fas fa-exclamation"></i> Confirmar?'; btnElement.classList.add('confirm-state');
...
async function enviarProducaoAPI(payload, cardId, btnElement) {
    if(btnElement) { btnElement.innerHTML = '<i class="fas fa-spinner fa-spin"></i>'; btnElement.style.pointerEvents = 'none'; }
    try {
        ...
        if(cardId) await fetch('/api/crm/deleteCard', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ sessionToken: localStorage.getItem('sessionToken'), cardId: cardId }) });
        showToast('Sucesso! Enviado para Produção.', 'success'); fecharPanel(); carregarKanban();
    } catch(err) {
...
document.getElementById('form-crm').addEventListener('submit', async (e) => {
    ...
    const originalText = btn.innerText; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Salvando...'; btn.disabled = true;
    const mats = []; document.querySelectorAll('.material-item').forEach(d => { const desc = d.querySelector('.mat-desc').value; if(desc) mats.push({ descricao: desc, detalhes: d.querySelector('.mat-det').value }); });
    const payload = {
        sessionToken: localStorage.getItem('sessionToken'), id: document.getElementById('card-id-db').value, titulo_manual: document.getElementById('crm-titulo-manual').value, nome_cliente: document.getElementById('crm-nome').value, wpp_cliente: document.getElementById('crm-wpp').value, servico_tipo: document.getElementById('pedido-servico-hidden').value, arte_origem: document.getElementById('pedido-arte-hidden').value, valor_orcamento: document.getElementById('crm-valor').value, valor_pago: document.getElementById('crm-valor-pago').value, valor_restante: document.getElementById('crm-valor-restante').value, briefing_json: JSON.stringify({ tipo_entrega: document.getElementById('pedido-entrega-hidden').value, materiais: mats, link_arquivo: document.getElementById('link-arquivo').value, supervisao_wpp: document.getElementById('pedido-supervisao').value, valor_designer: document.getElementById('valor-designer').value, formato: document.getElementById('pedido-formato').value, cdr_versao: document.getElementById('cdr-versao').value })
    };
    try {
    ...
    } catch(err) { showToast('Erro de conexão.', 'error'); } finally { btn.innerText = originalText; btn.disabled = false; }
});

window.abrirPanelEdicao = function(card) {
    resetarForm();
    document.getElementById('panel-titulo').innerText = 'Editar Oportunidade'; document.getElementById('display-id-automatico').innerText = card.titulo_automatico || '';
    document.getElementById('card-id-db').value = card.id; document.getElementById('crm-titulo-manual').value = card.titulo_automatico || ''; document.getElementById('crm-nome').value = card.nome_cliente; document.getElementById('crm-wpp').value = card.wpp_cliente;
    document.getElementById('crm-valor').value = card.valor_orcamento || 0; document.getElementById('crm-valor-pago').value = card.valor_pago || 0; calcularSaldoRestante();
    ...
    if(extras.link_arquivo) document.getElementById('link-arquivo').value = extras.link_arquivo; if(extras.supervisao_wpp) document.getElementById('pedido-supervisao').value = extras.supervisao_wpp; if(extras.valor_designer) document.getElementById('valor-designer').value = extras.valor_designer; if(extras.formato) document.getElementById('pedido-formato').value = extras.formato; if(extras.cdr_versao) document.getElementById('cdr-versao').value = extras.cdr_versao;
    const matContainer = document.getElementById('materiais-container'); matContainer.innerHTML = '';
    if(extras.materiais && extras.materiais.length > 0) extras.materiais.forEach(m => adicionarMaterialNoForm(m.descricao, m.detalhes)); else adicionarMaterialNoForm();
    currentStep = 1; renderizarPasso(); document.getElementById('slide-overlay').classList.add('active'); document.getElementById('slide-panel').classList.add('active');
};

window.mudarPasso = function(direction) { if (direction === 1 && !validarPassoAtual()) return; currentStep += direction; renderizarPasso(); };
function renderizarPasso() {
    document.querySelectorAll('.step-content').forEach(el => el.classList.remove('active')); document.getElementById(`step-${currentStep}`).classList.add('active');
    document.querySelectorAll('.step-indicator').forEach((ind, i) => { ind.classList.remove('active', 'completed'); if (i+1 < currentStep) ind.classList.add('completed'); if (i+1 === currentStep) ind.classList.add('active'); });
...
function validarPassoAtual() {
    if (currentStep === 1) { if (!document.getElementById('pedido-servico-hidden').value) { showToast("Selecione o serviço.", "error"); return false; } if (document.getElementById('crm-nome').value.length < 2) { showToast("Selecione ou busque um cliente.", "error"); return false; } }
...
window.selectCard = function(group, value, element) { document.getElementById(`pedido-${group}-hidden`).value = value; element.parentElement.querySelectorAll('.selection-card').forEach(c => c.classList.remove('selected')); element.classList.add('selected'); if (group === 'arte') { document.getElementById('arquivo-cliente-fields').classList.toggle('hidden', value !== 'Arquivo do Cliente'); document.getElementById('setor-arte-fields').classList.toggle('hidden', value !== 'Setor de Arte'); if(value === 'Setor de Arte') fetchSaldoCRM(); } };
...
function resetarForm() { document.getElementById('form-crm').reset(); document.getElementById('card-id-db').value = ''; document.querySelectorAll('.selection-card').forEach(c => c.classList.remove('selected')); ['pedido-servico-hidden', 'pedido-arte-hidden', 'pedido-entrega-hidden'].forEach(id => document.getElementById(id).value = ''); document.getElementById('crm-valor-restante').value = '0.00'; document.getElementById('materiais-container').innerHTML = ''; }
function adicionarMaterialNoForm(desc = '', det = '') { const container = document.getElementById('materiais-container'); const div = document.createElement('div'); div.className = 'material-item'; div.innerHTML = `<button type="button" class="btn-remove-material" onclick="this.closest('.material-item').remove()"><i class="fas fa-trash-alt"></i></button><div style="margin-bottom:10px"><input type="text" class="mat-desc form-control" value="${desc}" placeholder="Descrição"></div><div><textarea class="mat-det form-control" rows="2" placeholder="Detalhes...">${det}</textarea></div>`; container.appendChild(div); }
...
async function carregarKanban() {
    try {
        ...
        const cards = await res.json(); allCardsCache = cards; document.querySelectorAll('.kanban-items').forEach(c => c.innerHTML = '');
        cards.forEach(card => criarCardHTML(card)); inicializarDragAndDrop();
...
function criarCardHTML(card) {
    ...
    const div = document.createElement('div'); div.className = 'kanban-card'; div.dataset.id = card.id; div.onclick = (e) => { if(!e.target.closest('button')) abrirPanelEdicao(card); };
...
function inicializarDragAndDrop() {
    document.querySelectorAll('.kanban-items').forEach(col => {
        ...
        new Sortable(col, { group: 'crm', animation: 150, onEnd: function(evt) { fetch('/api/crm/moveCard', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ sessionToken: localStorage.getItem('sessionToken'), cardId: evt.item.dataset.id, novaColuna: evt.to.parentElement.dataset.status }) }); }});
...
async function fetchSaldoCRM() { const display = document.getElementById('crm-saldo-display'); if(!display) return; try { const res = await fetch('/api/crm/getBalance', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ sessionToken: localStorage.getItem('sessionToken') }) }); const data = await res.json(); display.innerText = parseFloat(data.saldo || 0).toLocaleString('pt-BR', {style:'currency', currency:'BRL'}); } catch(e) {} }