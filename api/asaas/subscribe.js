...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (!asaasCustomerId) {
            const customerPayload = { name: usuario.nome || `Usuário ${usuario.id || 'Teste'}`, cpfCnpj: usuario.cnpj || '00000000000' };
        ...
        if (!subscriptionId) {
            ...
            const subPayload = { customer: asaasCustomerId, billingType: 'PIX', nextDueDate: dueDate, value: valorAssinatura, cycle: 'MONTHLY', description: `Assinatura Mensal - Plataforma Setor de Arte - ${tipo.toUpperCase()}` };
        ...
        if (paymentsReq.data.data.length === 0) return res.status(200).json({ message: 'Assinatura Ativa.', status: 'ACTIVE' });

        const currentPaymentId = paymentsReq.data.data[0].id;
        const pixReq = await axios.get(`${ASAAS_BASE_URL}/payments/${currentPaymentId}/pixQrCode`, { headers: { 'access_token': ASAAS_API_KEY } });

        return res.status(200).json({ success: true, status: 'PENDING', pixUrl: pixReq.data.encodedImage, pixCode: pixReq.data.payload });

    } catch (error) {
        console.error("Erro na integração Asaas:", error.response ? error.response.data : error.message);
        return res.status(500).json({ message: 'Erro na integração Asaas.' });
    }
};
