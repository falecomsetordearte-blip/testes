...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (empresasLogadas.length === 0) return res.status(404).json({ message: 'Empresa não encontrada no banco de dados.' });
        ...
        if (arte === 'Setor de Arte') {
            ...
            if (saldoAtual < custoDesigner) {
                ...
                console.log(`[REQ-${reqId}] Bloqueio Saldo: Faltam R$ ${faltanteFmt}`);
        ...
        if (newDealId) {
            ...
            } catch (dbError) { console.error(`[REQ-${reqId}] ERRO DB:`, dbError.message); }
        }
