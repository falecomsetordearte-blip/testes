...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // 2. Busca as metas e os AJUSTES MANUAIS (Todas as colunas necessárias)
        ...
        // 3. Calcula Faturamento Real vindo dos Pedidos (Banco de Dados)
        ...
        // Inicializa com o valor real do banco
        let totalMesCompilado = parseFloat(faturamentoReal[0].real_mes || 0);
        let totalHojeCompilado = parseFloat(faturamentoReal[0].real_hoje || 0);
        let vendasSemanas = { sem1: 0, sem2: 0, sem3: 0, sem4: 0 };

        if (metas) {
            // --- LÓGICA DE SOMA DE AJUSTES SOLICITADA ---
            // Soma todos os ajustes manuais (Mês + Semanas) ao Total do Mês
            // Isso garante que o cálculo da meta diária considere tudo o que foi lançado manualmente
            const somaAjustes = 
                parseFloat(metas.ajuste_mes || 0) +
                parseFloat(metas.ajuste_sem_1 || 0) +
                parseFloat(metas.ajuste_sem_2 || 0) +
                parseFloat(metas.ajuste_sem_3 || 0) +
                parseFloat(metas.ajuste_sem_4 || 0);

            totalMesCompilado += somaAjustes;

            // Calcula faturamento real por semana para exibição individual das semanas
            ...
            // Atribui os valores das semanas somando seus respectivos ajustes individuais
            vendasSemanas.sem1 = parseFloat(realSemanas[0].r1 || 0) + parseFloat(metas.ajuste_sem_1 || 0);
            vendasSemanas.sem2 = parseFloat(realSemanas[0].r2 || 0) + parseFloat(metas.ajuste_sem_2 || 0);
            vendasSemanas.sem3 = parseFloat(realSemanas[0].r3 || 0) + parseFloat(metas.ajuste_sem_3 || 0);
            vendasSemanas.sem4 = parseFloat(realSemanas[0].r4 || 0) + parseFloat(metas.ajuste_sem_4 || 0);
        }

        return res.status(200).json({
            metas: metas,
            total_mes: totalMesCompilado, // Agora inclui Real + Ajuste Mês + Ajustes Semanas
