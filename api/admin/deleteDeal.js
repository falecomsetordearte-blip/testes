...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (!sessionToken || !dealId) {
            return res.status(400).json({ message: 'Sessão ou dealId ausentes.' });
        }

        // 1. Validar Token e Permissão Admin
        const user = await prisma.$queryRawUnsafe(`
            SELECT u.id, f.permissoes 
        ...
        if (!permissoes.includes('admin')) {
             return res.status(403).json({ message: 'Acesso Negado: Apenas administradores podem excluir pedidos permanentemente.' });
        }
        ...
        // 2. Excluir no banco de dados Neon
        await prisma.$executeRawUnsafe(`
            DELETE FROM pedidos WHERE id = CAST($1 AS INTEGER)
        `, pedidoId);

        res.status(200).json({ success: true, message: 'Pedido excluído com sucesso.' });

    } catch (error) {
        console.error('Erro ao excluir pedido no bd local:', error);
        res.status(500).json({ message: 'Erro ao excluir o pedido no servidor.', details: error.message });
    }
};
