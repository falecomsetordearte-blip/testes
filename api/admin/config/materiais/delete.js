...
module.exports = async (req, res) => {
    ...
    try {
        const { sessionToken, materialId } = req.body;
        if (!sessionToken) return res.status(401).json({ message: 'Sessão inválida' });
        if (!materialId) return res.status(400).json({ message: 'ID do material ausente.' });
        ...
        await prisma.$executeRawUnsafe(`
            UPDATE materiais SET ativo = false WHERE id = $1 AND empresa_id = $2
        `, parseInt(materialId), empresaId);

        return res.status(200).json({ message: "Material excluído/inativado com sucesso!" });

    } catch (error) {
        console.error("Erro POST deletar_material:", error);
